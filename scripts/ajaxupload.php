<?php
	...
	function uploadImage($fileName, $maxSize, $maxW, $fullPath, $relPath, $colorR, $colorG, $colorB, $maxH = null){
		...
		if($filesize > 0){	
			...
			if(count($errorList)<1){
				...
				foreach($allowed_ext as $ext){
					if($ext==end($file_ext)){
						...
						$newfilename = "temp-".$NUM.".".end($file_ext);
			...
			$errorList[]= "קובץ לא ניבחר";
		}
		if(!$match){
		   	$errorList[]= "סוג הקובץ אינו תקין: $filename";
		}
...
<?php
	if($imgUploaded){
		echo '<img src="'.$upload_image.'" border="0"  height="250px" width="250px" id="new_image" name="new_image" />';
	}else{
		echo 'Error(s) Found: ';
